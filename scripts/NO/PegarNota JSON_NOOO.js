...
function setImagenEnBox(boxName, filePath) {
  ...
  try {
    ...
    if (notas.length > 0) {
      ...
      var nota0 = notas[0];
      volanta  = cleanHTML(nota0.volanta  || "");
      titulo   = cleanHTML(nota0.titulo   || "");
      bajada   = cleanHTML(nota0.bajada   || "");
      firma    = cleanHTML(nota0.firma    || "");
      epigrafe = cleanHTML(nota0.epigrafe || "");
      cuerpo   = cleanHTML(nota0.cuerpo   || "");
    } else if (tiene_texto && fs.existsSync(rutaTXT)) {
